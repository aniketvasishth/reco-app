  ...
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) => {
      return cache.addAll(STATIC_ASSETS).catch((err) => {
        console.log('[Reco SW] Pre-cache partial:', err);
      });
    })
  );
  self.skipWaiting();
...
self.addEventListener('fetch', (event) => {
  // Only handle GET requests and skip API or cross-origin AI streams
  if (event.request.method !== 'GET') return;
  
  const url = new URL(event.request.url);
  if (url.pathname.startsWith('/api/')) return;

  // Network-First for HTML navigations to ensure latest status-bar theme-color & dynamic manifest
